@echo off
REM Auto test script: reads TOKEN from .env if present, else prompts.
...
curl -s -X POST %BASE%/api/opportunities -H "Authorization: Bearer %TOKEN%" -H "Content-Type: application/json" -d "{\"title\":\"Test Opp From Auto Script\",\"opportunityType\":\"International Internship\",\"status\":\"Draft\"}"
echo.
echo Done.
ENDLOCAL
